...
class Connection:
    ...
    def update(self):
        if(self.sending):
            ...
            aux_list[0] = int(
                self.lerp(self.sender[0], self.b.position[0], 0.1))
            aux_list[1] = int(
                self.lerp(self.sender[1], self.b.position[1], 0.1))
            self.sender = tuple(aux_list)
            distance = math.sqrt(
                (self.sender[0] - self.b.position[0])**2 +
                (self.sender[1] - self.b.position[1])**2)
            print(distance)
            if(distance < 15):
